...
a prompt_tokens property, showing the number of tokens in the prompt that was sent to the model; and
...
Assignment
After getting the response, but before printing the response text, print the number of tokens consumed by the interaction, in the following format:
...
Verify that you can see the token counts when you run your script. Every response from the model will be different, but it could look something like this:
